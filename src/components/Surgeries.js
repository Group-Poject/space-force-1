...
const Surgeries = props => {
  const [
    { surgery_name, surgery_desc, surgery_date },
    setValues,
    resetValues
  ] = useInputValue(
    {
  ...
  const getSurgeries = () =>
    axios
      .get(`/api/surgeries${props.user.patient_id}`)
  ...
  const edit = () => {
      surgery_list.map((e,i)=>{
          return <Edit key={i} inputs={e}/>
      })
    }
    ...
    <main id="meds-main">
      <form
        onSubmit={e => {
          e.preventDefault();
          axios
            .post(`/api/surgery${props.user.patient_id}`, {
              surgery_name,
              surgery_desc,
              surgery_date
            })
            ...
            <p className="med-action">
              ...
              <p>
                {toggle ? (
                  ...
                  <actionbutton onClick={() => setToggle(!toggle)}>
                    Edit
                  </actionbutton>
                )}
                <actionbutton
